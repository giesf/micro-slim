...
export const getRawBody = (req: IncomingMessage, options: any) => {
    return new Promise<Buffer>((resolve, reject) => {
        let received = 0;
        let { limit, length, encoding = 'utf-8' } = options;
        ...
        const onData = (chunk: any) => {
            ...
            if (limit != null && received > limit) {
                reject(createHTTPError(413, 'request entity too large', {
                    limit: limit,
                    received: received,
        ...
        const onEnd = () => {
            if (length != null && received !== length) {
                reject(createHTTPError(400, 'request size did not match content length', {
                    expected: length,
                    length: length,
                    received: received,
